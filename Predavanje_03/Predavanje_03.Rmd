...
2. Preberite podatke v mapi *data_raw* o BDP na prebivalca evropskih držav `EU_GDP_per_capita.csv`, ki smo jih pobrali 29. septembra 2021 iz spletne strani Eurostat (https://ec.europa.eu/eurostat/web/national-accounts/data/database). Pri nalaganju tabele uporabite parameter `stringsAsFactors = F`.

```{r iclude= FALSE, echo = FALSE}
BDP <- read.csv("data_raw/EU_GDP_per_capita.csv", sep=',', stringsAsFactors = F)
head(BDP)
```
...
- Preberite tabelo o BDP na prebivalca za Slovenijo in njene vrednosti dodajete v tabelo ostalih EU držav. Tudi tukaj pri nalaganju tabele uporabite parameter `stringsAsFactors = F`.
```{r echo = FALSE}
slo_BDP <- read.csv("data_raw/SLO_GDP_per_capita.csv", sep=',', stringsAsFactors = F)
